...
#ifdef DEBUG
#define VPROXY_CSUM_DEBUG 1
#endif

#ifdef VPROXY_CSUM_DEBUG
#include <stdio.h>
#endif

static inline int vproxy_pkt_ether_type(char* raw) {
    return ((raw[ETHER_TYPE_OFF] & 0xff) << 8) | (raw[ETHER_TYPE_OFF + 1] & 0xff);
}

static inline int vproxy_pkt_ether_type_with_vlan(char* raw) {
    return ((raw[ETHER_TYPE_WITH_VLAN_OFF] & 0xff) << 8) | (raw[ETHER_TYPE_WITH_VLAN_OFF + 1] & 0xff);
}

static inline int vproxy_pkt_ipv4_hdr_len(char* raw) {
    return (raw[IP4_IHL_OFF] & 0xf) * 4;
}

static inline int vproxy_pkt_ipv4_total_len(char* raw) {
    return ((raw[IP4_LEN_OFF] & 0xff) << 8) | (raw[IP4_LEN_OFF + 1] & 0xff);
}

static inline int vproxy_pkt_ipv4_proto(char* raw) {
    return raw[IP4_PROTO_OFF] & 0xff;
}

static inline int vproxy_pkt_ipv6_payload_len(char* raw) {
    return ((raw[IP6_PAYLOAD_LEN_OFF] & 0xff) << 8) | (raw[IP6_PAYLOAD_LEN_OFF + 1] & 0xff);
}

static inline int vproxy_pkt_ipv6_next_hdr(char* raw) {
    return raw[IP6_NEXT_HDR_OFF] & 0xff;
}

static inline int vproxy_pkt_ipv6_require_next_header(int next_header) {
    int n = next_header;
...
char* vproxy_pkt_ipv6_skip_to_last_hdr(char* raw, int len, char* proto) {
    int xh = vproxy_pkt_ipv6_next_hdr(raw);
    char* xh_buf = raw + IP6_HDR_MIN_LEN;
    int ex_off = IP6_HDR_MIN_LEN;
    while (vproxy_pkt_ipv6_require_next_header(xh)) {
        if (ex_off + 8 > len) {
            #ifdef VPROXY_CSUM_DEBUG
...
int vproxy_pkt_icmp4_csum(char* raw, int len, int flags, struct vproxy_csum_out* out) {
    ...
    if (!(flags & VPROXY_CSUM_UP) && !(flags & VPROXY_CSUM_UP_PSEUDO)) {
        return 0;
    }
    ...
    if (!(flags & VPROXY_CSUM_UP)) {
        return 0;
    }

    int csum = vproxy_csum_plain_calc(raw, len);
...
int vproxy_pkt_tcp4_csum(char* ip, char* raw, int len, int flags, struct vproxy_csum_out* out) {
    ...
    int csum;
    if (flags & VPROXY_CSUM_UP) {
        csum = vproxy_csum_ipv4_pseudo_calc(ip + IP4_SRC_OFF, ip + IP4_DST_OFF, IP_PROTOCOL_TCP, raw, len);
    } else {
        csum = vproxy_csum_ipv4_pseudo_calc(ip + IP4_SRC_OFF, ip + IP4_DST_OFF, IP_PROTOCOL_TCP, NULL, 0);
    }
...
int vproxy_pkt_udp4_csum(char* ip, char* raw, int len, int flags, struct vproxy_csum_out* out) {
    ...
    int csum;
    if (flags & VPROXY_CSUM_UP) {
        csum = vproxy_csum_ipv4_pseudo_calc(ip + IP4_SRC_OFF, ip + IP4_DST_OFF, IP_PROTOCOL_UDP, raw, len);
        if (csum == 0) {
            csum = 0xffff;
        }
    } else {
        csum = vproxy_csum_ipv4_pseudo_calc(ip + IP4_SRC_OFF, ip + IP4_DST_OFF, IP_PROTOCOL_UDP, NULL, 0);
    }
...
int vproxy_pkt_ipv4_csum(char* raw, int len, int flags, struct vproxy_csum_out* out) {
    ...
    int hdr_len = vproxy_pkt_ipv4_hdr_len(raw);
    ...
    int total_len = vproxy_pkt_ipv4_total_len(raw);
    ...
    if (flags & VPROXY_CSUM_IP) {
        raw[IP4_CSUM_OFF] = 0;
        raw[IP4_CSUM_OFF + 1] = 0;
        int csum = vproxy_csum_plain_calc(raw, hdr_len);
    ...
    int proto = vproxy_pkt_ipv4_proto(raw);
    if (proto == IP_PROTOCOL_ICMP) {
        return vproxy_pkt_icmp4_csum(upper, upper_len, flags, out);
        ...
        return vproxy_pkt_tcp4_csum(raw, upper, upper_len, flags, out);
    } else if (proto == IP_PROTOCOL_UDP) {
        return vproxy_pkt_udp4_csum(raw, upper, upper_len, flags, out);
    } else {
        #ifdef VPROXY_CSUM_DEBUG
...
int vproxy_pkt_icmp6_csum(char* ip, char* raw, int len, int flags, struct vproxy_csum_out* out) {
    ...
    int csum;
    if (flags & VPROXY_CSUM_UP) {
        csum = vproxy_csum_ipv6_pseudo_calc(ip + IP6_SRC_OFF, ip + IP6_DST_OFF, IP_PROTOCOL_ICMPv6, raw, len);
    } else {
        csum = vproxy_csum_ipv6_pseudo_calc(ip + IP6_SRC_OFF, ip + IP6_DST_OFF, IP_PROTOCOL_ICMPv6, NULL, 0);
    }
    raw[ICMP_CSUM_OFF] = (csum >> 8) & 0xff;
    raw[ICMP_CSUM_OFF + 1] = csum & 0xff;
    return 0;
...
int vproxy_pkt_tcp6_csum(char* ip, char* raw, int len, int flags, struct vproxy_csum_out* out) {
    ...
    int csum;
    if (flags & VPROXY_CSUM_UP) {
        csum = vproxy_csum_ipv6_pseudo_calc(ip + IP6_SRC_OFF, ip + IP6_DST_OFF, IP_PROTOCOL_TCP, raw, len);
    } else {
        csum = vproxy_csum_ipv6_pseudo_calc(ip + IP6_SRC_OFF, ip + IP6_DST_OFF, IP_PROTOCOL_TCP, NULL, 0);
    }
    raw[TCP_CSUM_OFF] = (csum >> 8) & 0xff;
    raw[TCP_CSUM_OFF + 1] = csum & 0xff;
    return 0;
...
int vproxy_pkt_udp6_csum(char* ip, char* raw, int len, int flags, struct vproxy_csum_out* out) {
    ...
    int csum;
    if (flags & VPROXY_CSUM_UP) {
        csum = vproxy_csum_ipv6_pseudo_calc(ip + IP6_SRC_OFF, ip + IP6_DST_OFF, IP_PROTOCOL_UDP, raw, len);
        if (csum == 0) {
            csum = 0xffff;
        }
    } else {
        csum = vproxy_csum_ipv6_pseudo_calc(ip + IP6_SRC_OFF, ip + IP6_DST_OFF, IP_PROTOCOL_UDP, NULL, 0);
    }
    raw[UDP_CSUM_OFF] = (csum >> 8) & 0xff;
    raw[UDP_CSUM_OFF + 1] = csum & 0xff;
    return 0;
...
int vproxy_pkt_ipv6_csum(char* raw, int len, int flags, struct vproxy_csum_out* out) {
    ...
    int payload_len = vproxy_pkt_ipv6_payload_len(raw);
    ...
    if (vproxy_pkt_ipv6_require_next_header(xh)) {
        char proto;
        upper = vproxy_pkt_ipv6_skip_to_last_hdr(raw, len, &proto);
        if (upper == NULL) {
            return 1;
        }
        xh = proto & 0xff;
    ...
    int upper_len = payload_len - (int)(upper - raw - IP6_HDR_MIN_LEN);
    if (xh == IP_PROTOCOL_ICMP) {
        return vproxy_pkt_icmp4_csum(upper, upper_len, flags, out);
    } else if (xh == IP_PROTOCOL_ICMPv6) {
        return vproxy_pkt_icmp6_csum(raw, upper, upper_len, flags, out);
    } else if (xh == IP_PROTOCOL_TCP) {
        return vproxy_pkt_tcp6_csum(raw, upper, upper_len, flags, out);
    } else if (xh == IP_PROTOCOL_UDP) {
        return vproxy_pkt_udp6_csum(raw, upper, upper_len, flags, out);
    } else {
        #ifdef VPROXY_CSUM_DEBUG
...
int vproxy_pkt_ether_csum_ex(char* raw, int len, int flags, struct vproxy_csum_out* out) {
    ...
    int ether_type = vproxy_pkt_ether_type(raw);
    ...
    if (ether_type == ETHER_TYPE_8021Q) {
        ...
        ether_type = vproxy_pkt_ether_type_with_vlan(raw);
    ...
    if (ether_type == ETHER_TYPE_IPv4) {
        return vproxy_pkt_ipv4_csum(ippkt, ip_len, flags, out);
    } else {
        return vproxy_pkt_ipv6_csum(ippkt, ip_len, flags, out);
    }
}
